...
const handleCanvasClick = ({ board, turn, removals }, { selected, canRemove, size }) => R.pipe(
  sanitizeMouseEvent,
  getPositionClicked({ board }, { size }),
  handlePositionClick({ board, turn, removals }, { selected, canRemove, size })
)

const handlePositionClick = ({ board, turn, removals }, { selected, canRemove }) => position => {
  if (canRemove) {
    if (belongsToPlayer (board, turn) (position)) {
      if (removals + 1 === 2) {
        return { pylos: [removeAction(position), commitAction()], ui: [disallowRemovalsAction()] }
      } else {
        return { pylos: [removeAction(position)] }
      }
    } else {
      return {}
    }
    ...
    if (selected) {
      if (R.includes(position, liftsFrom (board) (selected))) {
        if (hasSquare (move (selected, position) (board)) (position)) {
          return { pylos: [liftAction(selected, position)], ui: [allowRemovalsAction()] }
        } else {
          return { pylos: [liftAction(selected, position), commitAction()] }
        }
      ...
      if (isPlaceable (board) (position)) {
        if (hasSquare (insert (position, turn) (board)) (position)) {
          ...
          return { pylos: [insertAction(position, turn), commitAction()] }
        }
...
const canvasClickListener = (pylosStore, uiStore) => R.pipe(
  handleCanvasClick(pylosStore.getState(), uiStore.getState()),
